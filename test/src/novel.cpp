    ...
    bool load(const std::string& tileset, sf::Vector2u tileSize, const int* tiles, unsigned int width, unsigned int height) {
        ...
        for (unsigned int i = 0; i < width; ++i)
            for (unsigned int j = 0; j < height; ++j) {
                int tileNumber = tiles[i + j * width]; // on récupère le numéro de tuile courant
                int tu = tileNumber % (m_tileset.getSize().x / tileSize.x);// on en déduit sa position dans la texture du tileset
                int tv = tileNumber / (m_tileset.getSize().x / tileSize.x);

                sf::Vertex* quad = &m_vertices[(i + j * width) * 4]; // on récupère un pointeur vers le quad à définir dans le tableau de vertex
